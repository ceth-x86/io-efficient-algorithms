...
def create_simulator_from_array(arr: np.ndarray, block_size: int, memory_size: int) -> IOSimulator:
    flat = arr.flatten()
...
class TestExternalMergeSort:
    ...
    def test_successful_sorting(self, array_size: int, block_size: int, memory_size: int) -> None:
        ...
        disk = create_simulator_from_array(test_array, block_size=block_size, memory_size=memory_size)
        ...
        max_ios = max(200, int(theoretical_ios * 15))  # Allow 15x theoretical for implementation/eviction overhead
    ...
    def test_already_sorted_array(self) -> None:
        ...
        disk = create_simulator_from_array(test_array, block_size=4, memory_size=8)
        result, io_count = external_merge_sort(disk, array_size)
    ...
    def test_array_with_duplicates(self) -> None:
        ...
        disk = create_simulator_from_array(test_array, block_size=2, memory_size=6)
    ...
    def test_single_element_array(self) -> None:
        ...
        disk = create_simulator_from_array(test_array, block_size=1, memory_size=2)
    ...
    @pytest.mark.parametrize(
        ...
        [
            (8, 40),  # Small array - should be very efficient
            (16, 100),  # Medium array
            (32, 240),  # Larger array
        ],
    )
    ...
    def test_large_values_range(self) -> None:
        ...
        disk = create_simulator_from_array(test_array, block_size=5, memory_size=10)
        result, io_count = external_merge_sort(disk, array_size)
    ...
    def test_edge_case_block_sizes(self) -> None:
        ...
        disk1 = create_simulator_from_array(test_array.copy(), block_size=1, memory_size=3)
        result1, io_count1 = external_merge_sort(disk1, len(test_array))

        # Test with block_size = array_size (everything fits in one block)
        disk2 = create_simulator_from_array(test_array.copy(), block_size=len(test_array), memory_size=len(test_array) + 1)
