...
sim = IOSimulator(vd, block_size=2, memory_size=8)
...
for n in matrix_sizes:
    ...
    # Test cache-aware
    vd1 = VirtualDisk(size=n*n)
    vd1.disk = list(A.flatten())
    sim1 = IOSimulator(vd1, block_size=block_size, memory_size=memory_size)
    _, io_aware = transpose_cache_aware(sim1, n, n)
    
    # Test cache-oblivious
    vd2 = VirtualDisk(size=n*n)
    vd2.disk = list(A.flatten())
    sim2 = IOSimulator(vd2, block_size=block_size, memory_size=memory_size)
...
*Both algorithms perform equally well, with cache-oblivious showing advantages on non-power-of-2 matrices.*

### I/O Complexity Analysis: Theoretical vs Actual Results

Both algorithms achieve **T(n) = O(n/B)** I/O complexity. Here's how the actual results compare to theoretical bounds:

#### Cache-Aware Algorithm: O(n/B) Analysis

| Matrix | n  | B | Actual I/O | n/B | Ratio | Status | Analysis |
|--------|----|----|-----------|-----|-------|--------|----------|
| 3x3    | 9  | 1 | 18        | 9.0 | 2.00  | OK  | Expected factor ≈2 (read+write) |
| 4x4    | 16 | 2 | 16        | 8.0 | 2.00  | OK  | Optimal tiling efficiency |
| 4x4    | 16 | 2 | 24        | 8.0 | 3.00  | OK  | Larger tile size causes overhead |
| 5x5    | 25 | 1 | 50        | 25.0| 2.00  | OK  | Consistent 2x factor |
...
| Matrix | n  | B | Actual I/O | n/B | Ratio | Status | Analysis |
|--------|----|----|-----------|-----|-------|--------|----------|
| 2x2    | 4  | 1 | 4         | 4.0 | 1.00  | Perfect | Ideal recursive division |
| 3x3    | 9  | 1 | 14        | 9.0 | 1.56  | Excellent | Better than cache-aware |
| 4x4    | 16 | 2 | 16        | 8.0 | 2.00  | OK  | Power-of-2 optimality |
| 5x5    | 25 | 1 | 42        | 25.0| 1.68  | Excellent | Better than cache-aware |
| 8x8    | 64 | 2 | 64        | 32.0| 2.00  | OK  | Consistent power-of-2 |
...
|-------------|-----------------|---------------------|--------|-------------|
| 3x3         | 18              | 14                  | Cache-Oblivious | 22% better |
| 4x4         | 16              | 16                  | Tie             | Equal |
| 5x5         | 50              | 42                  | Cache-Oblivious | 16% better |

#### Key Insights

**Why Cache-Aware shows 2x factor:**
...
```bash
.venv/bin/pytest tests/test_transpose_cache_aware.py -v
.venv/bin/pytest tests/test_transpose_cache_oblivious.py -v
```
