...
def transpose_cache_oblivious(disk: IOSimulator, n_rows: int, n_cols: int) -> tuple[np.ndarray, int]:
    ...
    initial_io_count = wdisk.io_count

    # Start the recursive transpose (modifies the matrix in-place)
    _transpose_recursive(wdisk, 0, n_rows, 0, n_cols, n_cols)

    # For very small matrices, ensure we flush and do at least some I/O
    wdisk.flush_memory()

    # Calculate total I/O operations performed
    io_count = wdisk.io_count - initial_io_count

    # Return the transposed matrix
    return np.array(disk.disk.disk[:n_rows*n_cols]), io_count


def _transpose_recursive(
    disk: TransposeDiskWrapper, row_start: int, row_end: int, col_start: int, col_end: int, n_cols: int
) -> None:
    """
