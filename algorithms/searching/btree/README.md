...
- Traditional binary search trees require O(log_2 n) I/O operations per operation
...
- **Binary tree**: O(log_2 n) I/O per operation
- **B-tree**: O(log_B n) I/O per operation  
- **Improvement factor**: log_2(B) ≈ 3-10x for typical block sizes

**Example with n = 1M keys, B = 100:**
...
for n in sizes:
    # Create fresh B-tree
    vd = VirtualDisk(size=n * 10)
    disk = IOSimulator(vd, block_size=64, memory_size=256)
...
for d_min in [2, 4, 8, 16]:
    vd = VirtualDisk(size=10000)
    disk = IOSimulator(vd, block_size=d_min*4, memory_size=d_min*16)
...
.venv/bin/pytest tests/test_btree.py -v

# Run specific test categories
.venv/bin/pytest tests/test_btree.py::TestBTree::test_io_complexity -v
.venv/bin/pytest tests/test_btree.py::TestBTree::test_large_dataset -v
.venv/bin/pytest tests/test_btree.py::TestBTree::test_different_branching_factors -v
```
...
| Dataset Size | d_min | I/O Operations | Tree Height |
|--------------|-------|----------------|-------------|
| 100 keys     | 3     | ~15            | ~3          |
| 1,000 keys   | 3     | ~45            | ~4          |
| 10,000 keys  | 8     | ~80            | ~3          |
...
- **Binary tree height**: log_2(n) ≈ 13-17 levels for n=10,000-100,000
- **B-tree height**: log_B(n) ≈ 3-4 levels for same datasets  
- **I/O improvement**: 4-6x fewer I/O operations
