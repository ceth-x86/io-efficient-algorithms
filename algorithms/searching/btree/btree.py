...
class BTree:
    ...
    def _read_node(self, node_id: int) -> BTreeNode:
        ...
        # Read basic info
        stored_node_id = int(self.disk.read_element(start_pos))
        is_leaf = bool(int(self.disk.read_element(start_pos + 1)))
        num_keys = int(self.disk.read_element(start_pos + 2))
        ...
        for i in range(num_keys):
            key = self.disk.read_element(start_pos + 3 + i)
        ...
        num_children = int(self.disk.read_element(children_pos))

        for i in range(num_children):
            child_id = int(self.disk.read_element(children_pos + 1 + i))
