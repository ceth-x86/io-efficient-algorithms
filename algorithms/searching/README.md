...
- **[B-Trees](btree/)**: Classic balanced trees with O(log_B(n/B)) per operation
- **[Buffer Trees](buffer_tree/)**: Advanced batched processing achieving sorting bound O(n/B * log_{M/B}(n/M))
...
|---------------|------------------|-------------|---------------|
| **Binary tree** | O(log_2 n) | n * O(log_2 n) | Simple, small datasets |
| **B-tree** | O(log_B n) | n * O(log_B n) | Interactive queries, traditional databases |
| **Buffer tree** | N/A* | O(n/B * log_{M/B}(n/M)) | Batch processing, data warehouses |
