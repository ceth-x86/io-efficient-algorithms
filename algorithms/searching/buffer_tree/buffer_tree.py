...
class BufferTree:
    ...
    def _write_node(self, node: BufferTreeNode) -> None:
        ...
        for i, value in enumerate(data[:200]):  # Limit to reserved space
            if start_pos + i < self.disk.total_size:
                self.disk.write_element(start_pos + i, float(value))
    ...
    def _read_node(self, node_id: int) -> BufferTreeNode:
        ...
        # Read basic info
        stored_id = int(self.disk.read_element(start_pos))
        is_leaf = bool(int(self.disk.read_element(start_pos + 1)))
        degree = int(self.disk.read_element(start_pos + 2))
        num_keys = int(self.disk.read_element(start_pos + 3))
        num_children = int(self.disk.read_element(start_pos + 4))
        num_buffer = int(self.disk.read_element(start_pos + 5))
        num_data = int(self.disk.read_element(start_pos + 6))
        ...
        for i in range(num_keys):
            key = int(self.disk.read_element(pos + i))
            node.keys.append(key)
        pos += num_keys

        # Read children
        for i in range(num_children):
            child_id = int(self.disk.read_element(pos + i))
        ...
        for i in range(num_buffer):
            op_pos = pos + i * 5
            is_search = bool(int(self.disk.read_element(op_pos)))
            is_insert = bool(int(self.disk.read_element(op_pos + 1)))
            # is_delete not used in this implementation
            key = int(self.disk.read_element(op_pos + 3))
            value_hash = self.disk.read_element(op_pos + 4)
            # Reconstruct values from key
        ...
        for i in range(num_data):
            data_pos = pos + i * 2
            key = int(self.disk.read_element(data_pos))
            value_hash = self.disk.read_element(data_pos + 1)
    ...
    def _split_root_leaf(self, full_leaf: BufferTreeNode) -> None:
        ...
        # For simplicity, expand the capacity
