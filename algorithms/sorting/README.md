...
**Traditional 2-way merge**: O((n/B) * log_2(n/M)) - suboptimal by factor log_2(M/B)

**Our k-way merge**: O((n/B) * log_{M/B}(n/M)) - **optimal!**

The improvement comes from:
- **Larger branching factor**: k = M/B instead of 2
- **Shallower recursion tree**: log_{M/B} instead of log_2  
...
vd = VirtualDisk(size=len(data))
vd.disk = list(data.flatten())
sim = IOSimulator(vd, block_size=4, memory_size=8)
...
for n, block_size, memory_size in configurations:
    ...
    vd = VirtualDisk(size=n)
    vd.disk = list(data.flatten())
    sim = IOSimulator(vd, block_size=block_size, memory_size=memory_size)
...
| Array Size | Block Size | Memory Size | I/O Operations |
|------------|------------|-------------|----------------|
| 8          | 2          | 4           | ~12-16         |
| 16         | 4          | 8           | ~20-30         |
| 32         | 4          | 12          | ~40-60         |
